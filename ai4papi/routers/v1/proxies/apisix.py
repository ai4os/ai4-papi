...
session.headers.update(
    {"X-API-KEY": APISIX_API_KEY, "Content-Type": "application/json"}
)
...
def add_credential(user_id: str, name: str):
    ...
    credential_data = {"plugins": {"key-auth": {"key": token}}}

    resp = session.put(
        f"{APISIX_URL}/consumers/{user_id}/credentials/{name}", json=credential_data
    )
...
def get_api_keys(authorization=Depends(security)):
    ...
    resp = session.get(f"{APISIX_URL}/consumers/{auth_info['id']}/credentials")
...
def create_api_key(name: str, authorization=Depends(security)):
    """
    ...
    **Notes**:
    ...
    if auth_info["id"] not in consumers:
        ...
        consumer_data = {"username": auth_info["id"]}

        resp = session.put(f"{APISIX_URL}/consumers", json=consumer_data)

        if not resp.ok:
            raise HTTPException(
                status_code=resp.status_code, detail=json.loads(resp.text)
            )
...
def delete_api_key(name: str, authorization=Depends(security)):
    """
