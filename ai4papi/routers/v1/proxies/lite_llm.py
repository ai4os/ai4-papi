...
def update_teams(user_id, teams_to_add, teams_to_remove):
    ...
    for team in teams_to_add:
        ...
        if resp_add.ok == False:
            raise HTTPException(status_code=resp_add.status_code, detail=json.loads(resp_add.text))
    ...
    for team in teams_to_remove:
        ...
        if resp_delete.ok == False:
            raise HTTPException(status_code=resp_delete.status_code, detail=json.loads(resp_delete.text))
...
def get_user(user_id):
    ...
    if resp.ok:
        ...
        user = next(
            (u for u in data.get("users", []) if u.get("user_id") == user_id),
            None
        )
        return user
    else:
...
def create_user(user_id, user_email, teams):
    ...
    if resp_create.ok:
        ...
        raise HTTPException(status_code=resp_create.status_code, detail=json.loads(resp_create.text))
